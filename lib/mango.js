...
async function mangoIssues(mangoAddress, account) {
    const { host, port } = argv

    const mangoRepoLib = initLib(host, port, mangoAddress, account)

    return (await mangoRepoLib.issues())
            .map((issueFields, i) => Object.assign({ id: i}, createIssueFromFieldsArray(issueFields)))
}
...
async function mangoGetIssue(mangoAddress, account, issueId) {
    const { host, port } = argv

    const mangoRepoLib = initLib(host, port, mangoAddress, account)

    let issue = createIssueFromFieldsArray(await mangoRepoLib.getIssue(issueId))
    issue.id = issueId
    issue.content = (await ipfs.object.get(issue.hash)).toJSON().data.toString()

    return issue
}

function mangoNewIssueIpfs(mangoAddress, account, name, description, content, isActive = true) {
    const { host, port } = argv
    ...
    return mangoRepoLib.issueCount()
        .then(count => {
            ...
            return ipfs.object.put({ Data: content, Links: [] })
                .then(node => mangoRepoLib.newIssue(name, description, node.toJSON().multihash), isActive)
        })
}
...
function mangoStakeIssue(mangoAddress, account, issueId, stake) {
    const { host, port } = argv
